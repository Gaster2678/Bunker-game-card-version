...
class dead():
    ...
    async def main(list_of_players):
        ...
        room_id = await dead.check_room(user_id=list_of_players[0][0])
        list_players_vote = await dead.check_vote(room_id=room_id)
        player_dead = await dead.logik_max_vote(list_players_vote=list_players_vote)
        nik = await dead.nik_player(player=player_dead)
